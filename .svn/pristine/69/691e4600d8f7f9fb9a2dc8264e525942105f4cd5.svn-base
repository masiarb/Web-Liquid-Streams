...
/////////////
   ...
   // General:
   #define MOD_NAME "OverCpu for Node.js v0.4a"
...
///////////
   ...
   // Stats:
   unsigned long long jiffies, totJiffies, relTotJiffies, idleJiffies;
   float cpuUsage, relCpuUsage, totCpuUsage;
...
//////////
   ...
   bool updateStats()
   {
      ...
      // Tot CPU idle:
      unsigned long long prevIdleJiffies = idleJiffies;
      idleJiffies = cat[MOD_JIFFY_IDLE];
      unsigned long long idleDelta = idleJiffies - prevIdleJiffies;
      ...
      else
      {
         ...
         totCpuUsage = (1.0f - ((float) ((double)idleDelta) / ((double)delta))) * 100.0f;
      }
   ...
   static void init(Handle<Object> target)
   {
      ...
      NODE_SET_PROTOTYPE_METHOD(persFuncTemp, "getTotCpuUsage", getTotCpuUsage);
    ...
    * @param args instance wrapper
    */
   static Handle<Value> getTotCpuUsage(const Arguments& args)
   {
      HandleScope scope;
      OverCpu* overcpu = ObjectWrap::Unwrap<OverCpu>(args.This());
      ...
      // Return value:
      MOD_SYNC(overcpu->mutex);
      Local<Number> result = Number::New(overcpu->totCpuUsage);
      return scope.Close(result);
   }
